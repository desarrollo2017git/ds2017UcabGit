...
{
    public class NotificacionesDAO : Dao, INotificacionesDAO
    {
        ...
        internal ResultadoProceso Borrar(string codigo)
        {
            ...
            try
            {
                ...
                using (var db = new ContextoBD())
                {
                    ...
                    {
                        this.Borrar<Notificacion>(db.Notificaciones, notificacion);
                    }
                    else
                    {
            ...
            try
            {
                using (var db = new ContextoBD())
                {
                    if (notificacion.NotificacionId > 0)
                    {
                        ...
                        this.Actualizar<Notificacion>(db.Notificaciones, notificacion, notificacion.NotificacionId);
                    }
                    else
                        this.Crear<Notificacion>(db.Notificaciones, notificacion);
                }
                resultado.SinProblemas = true;
            }
            catch (Exception ex)
